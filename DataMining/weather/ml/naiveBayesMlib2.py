...
def extract_features(row, columnName):
    features = [float(row.max_temp)] + [float(row.med_temp)] + [float(row.min_temp)] + [float(row.max_pressure)] + [
...
def insertDataIntoDatabase(dataToInsert):
    ...
    for index, row in pandas_df.iterrows():
        ...
        print("StationId: " + station_id + "\tReal province:" + str(province) + "\tPredicted Province: " + str(
            predictedProvince) + "\tStationIndex Val:" + str(stationIndex) + "\tPredicted Val: " + str(prediction))
        session.execute(
            "INSERT INTO Station_Naive_Bayes_Prediction (station_id,\"stationIndex\",prediction,province) VALUES (%s, %s, %s, %s)",
            [str(station_id), stationIndex, prediction, str(province)])


def doBayes(data):
    groupedData = data.groupBy("stationIndex", "station_id").agg(avg("max_temp"), avg("med_temp"), avg("min_temp"),
                                                                 avg("max_pressure"), avg("min_pressure"),
                                                                 avg("precip"), avg("insolation"))
    selectedData = groupedData.select("station_id", "stationIndex", "avg(max_temp)", "avg(med_temp)", "avg(min_temp)",
                                      "avg(max_pressure)", "avg(min_pressure)", "avg(precip)", "avg(insolation)")

    columnsToPredict = ["stationIndex", "avg(max_temp)", "avg(med_temp)", "avg(min_temp)", "avg(max_pressure)",
                        "avg(min_pressure)", "avg(precip)", "avg(insolation)"]
