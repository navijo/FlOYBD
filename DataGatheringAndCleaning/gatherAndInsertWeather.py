...
def insertClimaticMonthlyData(row, data, anual, year):
	...
	visibility_under_50m = float(str(data.get('nv_0050')).replace(",",".")) if not (data.get('nv_0050') is None) else None
	visibility_under_100m = float(str(data.get('nv_0100')).replace(",",".")) if not (data.get('nv_0100') is None) else None
	visibility_under_1000m = float(str(data.get('nv_01000')).replace(",",".")) if not (data.get('nv_01000') is None) else None
	...
	lower_max_temperature = float(str(data.get('ti_max')).replace(",",".")) if not (data.get('ti_max') is None) else None
	...
	high_min_temperature = float(str(data.get('ts_min')).replace(",",".")) if not (data.get('ts_min') is None) else None
	...
	session.execute("INSERT INTO Monthly_Measurement (station_id,steam_press,measure_date,evap,relative_humidity,\
					...
					insolation_hours,covered_days,clear_days,foggy_days,hail_days,rain_days,snow_days,cloud_days,
...
def getMonthlyValue(fromYear):
	...
	while(year <= 2017):
		...
		for row in rows:
			...
			stationClimatics = getData("/valores/climatologicos/mensualesanuales/datos/anioini/"+str(year)+"/aniofin/"+str(year)+"/estacion/"+str(row[0]))

			if((type(stationClimatics) is not list) or ((stationClimatics is None) and stationClimatics.get("estado") == 404)):
				continue
...
def getClimatologicalDaily(yearFrom, monthFrom, dayFrom, yearTo, monthTo, dayTo):
	...
	for row in rows:
		...
		dailyClimatics = getData("/valores/climatologicos/diarios/datos/fechaini/"+fechaIni+"/fechafin/"+fechaFin+"/estacion/"+str(row[0]))
		...
		for day in range(0, len(dailyClimatics)):
			...
			printLog("Processing day ", day, " for station "+str(row[0]))
...
def getClimatologicalDailyAllYear(yearFrom, yearTo):
	...
	while(yearFrom <= yearTo):
		...
		while(dayOfTheYear <= 365):
			...
			# time.sleep(60)
			# print ("wake up..")
			dailyClimatics = getData("/valores/climatologicos/diarios/datos/fechaini/"+fechaIni+"/fechafin/"+fechaFin+"/todasestaciones")
